...
import { ShoppingBasket, ChevronLeft, ChevronRight, X, RefreshCw, Filter, Share2 } from 'lucide-react';
          ...
          <div key={ad.id} className={styles.listItem}>
            ...
            <div className={styles.cellActions}>
              <button 
                ...
                onClick={() => onShare(ad.phoneNumber)} 
                aria-label="Paylaş"
                title="Paylaş"
              >
                <Share2 size={18} />
  ...
  const handleShare = async (phoneNumber: string) => {
    ...
    try {
      if (navigator.share) {
        ...
        await navigator.clipboard.writeText(`${phoneNumber} - ${window.location.href}`);
        alert('Link köçürüldü!');
      }
    } catch (error) {
